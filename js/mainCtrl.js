angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  // The important thing to note is the 'data' property.
  // The value is 'songData'.
  // That means ng-grid is looking for songData on $scope
  //  and is putting whatever songData is into the grid.
  ...
  // so our user can see the results they get back from itunes.
    $scope.getSongData = function() {
      itunesService.getArtist($scope.artist).then(function(response){
        ...
        $scope.songData = parsedResponseArr;
      })
    }
// Write a function that will call the method on the itunesService that is responsible for getting the data from iTunes,
  ...
  */
  ...
  // Create a new array and then loop through the iTunes data pushing into
  // your new array objects that look like the above data.

    //Code here


  //Once you have that final data array, you simply need to put it on the scope
  // (or more specifically on the scope as songData).
